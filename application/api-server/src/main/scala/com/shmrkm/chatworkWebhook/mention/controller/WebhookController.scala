...
import akka.http.scaladsl.server.{Directive0, MalformedRequestContentRejection, Route}
...
class WebhookController(implicit system: ActorSystem) extends Controller {
  ...
  def extractRequestBody(entity: HttpEntity)(implicit ec: ExecutionContext): Future[String] =
      ...
      headerValueByName(signatureHeaderKey) { applySignature =>
        extractExecutionContext { implicit ec =>
          extractRequestEntity { requestEntity =>
            onSuccess(extractRequestBody(requestEntity)) { body =>
              verifySignature(applySignature, body) {
                entity(as[WebhookRequest]) { request =>
                  onComplete(execute(request.mentionCommand)) { response => complete(response) }
                }
              }
            }
          }
